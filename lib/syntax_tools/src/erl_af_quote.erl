...
parse_transform(Forms, _Options) ->
    ...
    QuoteAttr = erl_af_lib:analyze_forms_attributes(erl_af_quote, Forms),
    ...
    Opts = #{traverse => pre, formatter => ?MODULE, parse_transform => true, transform => Transform},
    erl_af:map(
        fun(Node, Attr) ->
...
format_error({invalid_unquote_splicing, Binding, Var}) ->
    io_lib:format("expected unquote, not unquote_splicing ~s in ~s",
                  [erl_af_lib:ast_safe_to_string(Binding), erl_af_lib:ast_safe_to_string(Var)]);
format_error({non_empty_tail, Rest}) ->
    io_lib:format("non empty expression '~s' after unquote_splicing in pattern", [erl_af_lib:ast_safe_to_string(Rest)]);
format_error({invalid_quote, Node}) ->
    io_lib:format("invalid quote ~s", [erl_af_lib:ast_safe_to_string(Node)]);
format_error(Message) ->
    erl_af:format_error(Message).

debug_forms(Forms1, QuoteAttr) ->
    case QuoteAttr of
        [debug] ->
            io:format("~s~n", [erl_af_lib:ast_to_string(Forms1)]);
        _ ->
            ok
    end.
...
quote(Value, Options, Node, Attr, #{file := File, module := Module}) ->
    ...
    erl_af_traverse_m:set_updated(quote_0(Value, Options1)).

quote_0(Value, #{debug := true, quote_line := QuoteLine, file := File} = Options) ->
    erl_af_monad:lift_m(
      fun(QuotedAst) ->
              QuotedCode = erl_af_lib:ast_safe_to_string(QuotedAst),
              RelaPath = erl_af_lib:relative_path(File),
              io:format("~ts:~p~n~s~n", [RelaPath, QuoteLine, QuotedCode]),
              QuotedAst
      end, quote_0(Value, Options#{debug => false}), erl_af_traverse_m);
quote_0(Value, #{line := Line, quote_line := QuoteLine} = Options) ->
    Options1 = maps:remove(line, Options),
    erl_af_monad:lift_m(
      fun(Quoted) ->
              call_remote(erl_af_lib, replace_line_zero, [Quoted, Line], QuoteLine)
      end, quote_0(Value, Options1), erl_af_traverse_m);
quote_0(Value, #{warnings := Warnings} = Options) ->
    Options1 = maps:remove(warnings, Options),
    erl_af_traverse_m:then(
      erl_af_traverse_m:warnings(Warnings),
...
quote_1({match, _Line1, Pattern, Value}, #{quote_line := Line, quote_type := pattern} = Opts) ->
    ...
    erl_af_traverse_m:bind(
      ...
      fun(Pattern1) ->
              erl_af_traverse_m:return({match, Line, Pattern1, Value})
      end);
        ...
        {value_list, Unquotes} ->
            erl_af_traverse_m:then(
                erl_af_traverse_m:update_line(
                  Line, erl_af_traverse_m:warning({invalid_unquote_splicing, Unquotes, Var})),
              quote_tuple(Var, Opts));
        {BindingType, Unquote} ->
...
quote_1({LiteralType, Line, Literal}, Opts) 
  when LiteralType == atom ; 
       ...
       LiteralType == float ; 
       LiteralType == string ->
    erl_af_traverse_m:return(quote_literal(LiteralType, Line, Literal, Opts#{quote_line => Line}));
quote_1(Tuple, Opts) when is_tuple(Tuple) ->
    quote_tuple(Tuple, Opts);
quote_1(List, Opts) when is_list(List) ->
    quote_list(List, Opts);
quote_1(Atom, Opts) when is_atom(Atom) ->
    erl_af_traverse_m:return(quote_atom(Atom, Opts));
quote_1(Integer, Opts) when is_integer(Integer) ->
    erl_af_traverse_m:return(quote_integer(Integer, Opts)).
    ...
    erl_af_traverse_m:bind(
      ...
      fun(H1) ->
              erl_af_traverse_m:bind(
                quote_1(T, Opts),
                fun(T1) ->
                        erl_af_traverse_m:return({cons, Line, H1, T1})
                end)
      end);
quote_list([], #{quote_line := Line}) ->
    erl_af_traverse_m:return({nil, Line}).
...
quote_tuple(Tuple, Opts) ->
    ...
    erl_af_traverse_m:bind(
      ...
      fun(TupleList1) ->
              erl_af_traverse_m:return({tuple, TupleLine, TupleList1})
      end).
    ...
    erl_af_traverse_m:bind(
      ...
      fun(MA1) ->
              erl_af_traverse_m:bind(
                quote_1(Spec, NOpts),
                fun(Spec1) ->
                        erl_af_traverse_m:return([MA1, Spec1])
                end)
      end);
...
quote_tuple_list([Action, TupleLine|Rest] = TupleList, Opts) when is_atom(Action), is_integer(TupleLine) ->
    ...
    erl_af_traverse_m:bind(
      ...
      fun(Rest1) ->
              ...
              erl_af_traverse_m:return([Action1, Line1|Rest1])
      end);
...
unquote(Exp, #{type := value}) ->
    erl_af_traverse_m:return(Exp);
unquote(Exp, #{type := Type, quote_line := Line} = Opts) ->
    erl_af_traverse_m:return({tuple, Line, [quote_atom(Type, Opts), quote_line(Opts), Exp]}).
...
unquote_splicing(Unquotes, [], #{quote_type := pattern, join := list}) ->
    erl_af_traverse_m:return(Unquotes);
...
unquote_splicing(Unquotes, Rest, #{quote_type := pattern, quote_line := Line}) ->
    ...
    erl_af_traverse_m:then(
      erl_af_traverse_m:update_line(
        Line, erl_af_traverse_m:warning(Warning)),
      erl_af_traverse_m:return(Unquotes)).
...
validate_options(Line, _QuoteLine) when is_integer(Line) ->
    #{code_line => Line};
validate_options(Options, Line) ->
    Return = erl_af_lib:validate(#{debug => boolean, code_line => boolean, line => any}, Options),
    erl_af_return:with_error(
        fun(ErrorStruct) ->
            erl_af_error:update_line(Line, ?MODULE, ErrorStruct)
        end, Return).
          ...
          fun(Node, _Attr) ->
                  ...
                  case lists:member(Type, StaticTypes) of
                      true ->
                          Node;
                      false ->
                         {continue, erl_af_lib:abstract_form(Node)}
                  end
          end, AstOptions, #{traverse => pre, simplify_return => true}),
    erl_syntax:abstract(AstOptions1).
