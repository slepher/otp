...
%%% @doc
%%% return type of {@link erl_af:map/3}, {@link erl_af:reduce/4}, {@link erl_af:map_with_state/4}, {@link erl_af:mapfold/4}.
%%% @end
%%% Created :  8 Jul 2020 by Chen Slepher <slepheric@gmail.com>
%%%-------------------------------------------------------------------
-module(erl_af_return).

-include("af_struct_name.hrl").

-export_type([struct/1]).

-type struct(A) :: erl_af_ok(A) | erl_af_return_error().

-type erl_af_ok(A) :: #{?STRUCT_KEY := ?RETURN_OK,
                                return := A,
                                error := erl_af_error:struct()}.

-type erl_af_return_error() :: #{?STRUCT_KEY := ?RETURN_FAIL,
                                  error := erl_af_error:struct()}.
...
-spec ok(A) -> struct(A).
ok(Return) ->
    ok(Return, erl_af_error:new()).

%% @spec ok(A, erl_af_error:struct()) -> struct(A)
-spec ok(A, erl_af_error:struct()) -> struct(A).
...
-spec fail() -> struct(ok).
fail() ->
    fail(erl_af_error:new()).

-spec fail(erl_af_error:struct()) -> struct(ok).
...
from_compiler_1({warning, Forms, Warnings}) when is_list(Warnings) ->
    ErrorStruct = erl_af_error:new(),
    ErrorStruct1 = erl_af_error:append_file_warnings(Warnings, ErrorStruct),
    {ok, ok(Forms, ErrorStruct1)};
from_compiler_1({error, Errors, Warnings}) when is_list(Warnings), is_list(Errors) ->
    ErrorStruct = erl_af_error:new(),
    ErrorStruct1 = erl_af_error:append_file_warnings(Warnings, ErrorStruct),
    ErrorStruct2 = erl_af_error:append_file_errors(Errors, ErrorStruct1),
...
from_walk_return(#{?STRUCT_KEY := ?WALK_RETURN, return := Return, errors := Errors, warnings := Warnings}) ->
    {ok,
     erl_af_return:then(
       erl_af_return:then(
         erl_af_return:warnings(Warnings),
         erl_af_return:errors(Errors)),
       erl_af_return:return(Return))};
from_walk_return(_) ->
    error.

to_monad(A) ->
    erl_af_lib:concerete(A, [fun from_return/1, fun from_walk_return/1, fun from_compiler_1/1]).

-spec to_compiler(struct([erl_syntax:syntaxTree()])) ->
                         [erl_syntax:syntaxTree()] | {warning, [erl_syntax:syntaxTree()], erl_af_error:compiler_error()} |
                         {error, erl_af_error:compiler_error(), erl_af_error:compiler_error()}.
to_compiler(#{?STRUCT_KEY := ?RETURN_OK, return := Forms, error := Error}) ->
    case erl_af_error:realize(Error) of
        {[], []} ->
            Forms;
        {[], Warnings} ->
...
to_compiler(#{?STRUCT_KEY := ?RETURN_FAIL, error := Error}) ->
    {Errors, Warnings} = erl_af_error:realize(Error),
    {error, Errors, Warnings}.

simplify(#{?STRUCT_KEY := ?RETURN_OK, return := Return, error := Error}) ->
    case erl_af_error:is_empty(Error) of
        true ->
            Return;
        false ->
            exit(erl_af_error:printable(Error))
    end;
...
errors(Errors) ->
    ErrorStruct = erl_af_error:new(),
    ErrorStruct1 = erl_af_error:append_errors(Errors, ErrorStruct),
    ok(ok, ErrorStruct1).
...
warnings(Warnings) ->
    ErrorStruct = erl_af_error:new(),
    ErrorStruct1 = erl_af_error:append_warnings(Warnings, ErrorStruct),
    ok(ok, ErrorStruct1).

has_error(#{error := ErrorStruct}) ->
    not erl_af_error:is_empty_error(ErrorStruct).
%%%===================================================================
%%% Internal functions
%%%===================================================================
bind_1(#{?STRUCT_KEY := ?RETURN_OK, return := A, error := Error0}, KMB) ->
    #{error := Error1} = MB = to_monad(KMB(A)),
    Error2 = erl_af_error:merge(Error0, Error1),
    MB#{error => Error2};
