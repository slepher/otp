...
-define(TRAVERSE_ERROR, erl_syntax_traverse_error).
...
add_attribute(F, AttrValues, Acc) when is_list(AttrValues) ->
    erl_syntax_traverse_m:then(
      erl_syntax_traverse_m:put(Acc),
      erl_syntax_monad:map_m(
        fun(AttrValue) ->
                erl_syntax_traverse_m:bind(
                  erl_syntax_traverse_m:get(),
                  fun(Acc1) ->
                          add_attribute(F, AttrValue, Acc1)
                  end)
        end, AttrValues, erl_syntax_traverse_m));
              ...
              fun(NodeB) ->
                      case SyntaxLib:tps(NodeB, Opts) of
                          {_Type, _Pos, []} ->
                              ...
                              %% erl_syntax_traverse_m:to_node(erl_syntax_traverse_m:return(NodeB));
...
fun_return_to_monad(Return, Node, Opts) ->
    ...
    case erl_syntax_walk_return:to_map(Return) of
        {ok, StructBase} ->
            WalkReturn = erl_syntax_walk_return:new(StructBase),
            erl_syntax_traverse_m:erl_syntax_traverse_m(WalkReturn);
        error ->
            case {Return, WithState} of
                {{Node1, State}, true} ->
                    WalkReturn = erl_syntax_walk_return:new(#{node => Node1, state => State}),
                    erl_syntax_traverse_m:erl_syntax_traverse_m(WalkReturn);
                _ ->
