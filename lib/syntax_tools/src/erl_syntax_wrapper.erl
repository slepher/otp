...
subtrees(Node, #{type := Type} = Opts) ->
    try erl_syntax:subtrees(Node) of
        ...
        EType:{badarg, _}:StackTrace ->
            erlang:raise(EType, {invalid_node, Node, Opts}, StackTrace)
    end;
subtrees(Node, #{} = Opts) ->
...
update_subtrees(Node, Subtrees) ->
    try erl_syntax:revert_root(erl_syntax:update_tree(Node, Subtrees)) of
        Node1 ->
            Node1
    catch
        Type:Exception:StackTrace ->
            erlang:raise(Type, {update_subtress_failed, Node, Subtrees, Exception}, StackTrace)
    end.
...
default_revert_body_trees(Name, BodyTrees) ->
  Bodies = lists:map(fun(BodyTree) -> erl_syntax:revert_root(BodyTree) end, BodyTrees),
