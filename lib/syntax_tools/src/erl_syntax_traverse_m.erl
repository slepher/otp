...
-define(STATE_OK, erl_syntax_traverse_m_state_ok).
-define(STATE_FAIL, erl_syntax_traverse_m_state_fail).
...
%%% types
%%%===================================================================
-export_type([erl_syntax_traverse_m/2]).

-opaque erl_syntax_traverse_m(S, A) :: 
          #{?STRUCT_KEY => ?TRAVERSE_M, inner => erl_syntax_traverse_m_inner(S, A)}.

-type erl_syntax_traverse_m_inner(S, A) :: 
        fun((erl_syntax_error_state:formatter(), S, erl_syntax_error_state:erl_syntax_error_state()) ->
                   erl_syntax_traverse_m_state(S, A)).

-type erl_syntax_traverse_m_state(S, A) :: 
        #{?STRUCT_KEY => ?STATE_OK,
          return => A,
          nodes => syntax_nodes(),
          updated => boolean(),
          continue => boolean(),
          state => S, 
          error => erl_syntax_error_state:erl_syntax_error_state()} |
...
-type endo(A) :: erl_syntax_endo:endo(A).

%%%===================================================================
%%% API
%%%===================================================================
-export([erl_syntax_traverse_m/1, to_monad/1, to_monad/2]).
...
%%% API
%%%===================================================================
-spec erl_syntax_traverse_m(erl_syntax_walk_return:erl_syntax_walk_return(S, A) |
                           erl_syntax_base_m:erl_syntax_base_m(A) |
                           erl_syntax_traverse_m_state(S, A) |
                           erl_syntax_return_m:erl_syntax_return_m(A)) ->
    erl_syntax_traverse_m(S, A).
erl_syntax_traverse_m(#{?STRUCT_KEY := ?WALK_RETURN} = Map) ->
    Inner =
        fun(_Formatter, State, Error0) ->
                ...
                Error1 = erl_syntax_error_state:append_ews(Errors, Warnings, Error0),
                case Errors of
                    [] ->
                        state_ok(#{return => Return, state => State1, nodes => erl_syntax_endo:endo(Nodes),
    ...
    new(Inner);
erl_syntax_traverse_m(#{?STRUCT_KEY := ?BASE_M, return := Return, errors := Errors, warnings := Warnings}) ->
    Inner =
        fun(_Formatter, State, Error0) ->
                Error1 = erl_syntax_error_state:append_ews(Errors, Warnings, Error0),
    ...
    new(Inner);
erl_syntax_traverse_m(#{?STRUCT_KEY := ?RETURN_OK, return := Return, error := Error1}) ->
    Inner =
        fun(_Formatter, State, Error0) ->
                Error2 = erl_syntax_error_state:merge(Error0, Error1),
                state_ok(#{return => Return, state => State, error => Error2})
        end,
    new(Inner);
erl_syntax_traverse_m(#{?STRUCT_KEY := ?RETURN_FAIL, error := Error1}) ->
    Inner =
        fun(_Formatter, State, Error0) ->
                Error2 = erl_syntax_error_state:merge(Error0, Error1),
    ...
    new(Inner);
erl_syntax_traverse_m(#{?STRUCT_KEY := ?TRAVERSE_M} = MA) ->
    MA.
...
-spec to_monad(term()) -> erl_syntax_traverse_m(term(), term()).
to_monad(A) ->
    to_monad(ok, A).

-spec to_monad(A, term()) -> erl_syntax_traverse_m(term(), A).
to_monad(A, Return) ->
    Return1 = 
        case convertable_struct(Return) of
            true ->
                Return;
            false ->
                case erl_syntax_walk_return:to_map(A, Return) of
                    {ok, StructBase} ->
                        erl_syntax_walk_return:new(StructBase);
                    error ->
                        erl_syntax_walk_return:new(#{return => Return})
                end
        end,
    erl_syntax_traverse_m:erl_syntax_traverse_m(Return1).

-spec new(erl_syntax_traverse_m_inner(S, A)) -> erl_syntax_traverse_m(S, A).
...
-spec run(erl_syntax_traverse_m(S, A), formatter(), S) -> erl_syntax_return_m:erl_syntax_return_m({A, S}).
run(#{?STRUCT_KEY := ?TRAVERSE_M} = MA, Formatter, State) ->
    erl_syntax_monad:lift_m(fun({A, State1, _Nodes}) -> {A, State1} end, run_1(MA, Formatter, State), erl_syntax_return_m).

-spec run_1(erl_syntax_traverse_m(S, A), formatter(), S) -> erl_syntax_return_m:erl_syntax_return_m({A, S, [any()]}).
run_1(#{?STRUCT_KEY := ?TRAVERSE_M} = MA, Formatter, State) ->
    case run_0(MA, Formatter, State, erl_syntax_error_state:new()) of
        #{?STRUCT_KEY := ?STATE_OK, return := Return, state := State1, nodes := Nodes, error := Error} ->
            erl_syntax_return_m:return_ok({Return, State1, erl_syntax_endo:run(Nodes)}, Error);
        #{?STRUCT_KEY := ?STATE_FAIL, error := Error} ->
            erl_syntax_return_m:return_fail(Error)
    end.

-spec eval(erl_syntax_traverse_m(S, A), formatter(), S) -> erl_syntax_return_m:erl_syntax_return_m(A).
eval(#{?STRUCT_KEY := ?TRAVERSE_M} = MA, Formatter, State) ->
    erl_syntax_monad:lift_m(fun({A, _State, _Nodes}) -> A end, run_1(MA, Formatter, State), erl_syntax_return_m).

-spec exec(erl_syntax_traverse_m(S, _A), formatter(), S) -> erl_syntax_return_m:erl_syntax_return_m(S).
exec(#{?STRUCT_KEY := ?TRAVERSE_M} = MA, Formatter, State) ->
    erl_syntax_monad:lift_m(fun({_A, State1, _Nodes}) -> State1 end, run_1(MA, Formatter, State), erl_syntax_return_m).

-spec enodes(erl_syntax_traverse_m(S, _A), formatter(), S) -> erl_syntax_return_m:erl_syntax_return_m(syntax_nodes()).
enodes(#{?STRUCT_KEY := ?TRAVERSE_M} = MA, Formatter, State) ->
    F = fun({_A, _State1, Nodes}) -> Nodes end,
    erl_syntax_monad:lift_m(F, run_1(MA, Formatter, State), erl_syntax_return_m).

-spec bind(erl_syntax_traverse_m(S, A), fun((A) -> erl_syntax_traverse_m(S, B))) -> erl_syntax_traverse_m(S, B).
      ...
      fun(Formatter, #{return := A, state := State1,
              ...
              case MB of
                  #{?STRUCT_KEY := ?STATE_OK, updated := Updated2, continue := Continue2, nodes := Nodes2} ->
                      Nodes3 = erl_syntax_endo:append(Nodes1, Nodes2),
...
-spec then(erl_syntax_traverse_m(S, _A), erl_syntax_traverse_m(S, B)) -> erl_syntax_traverse_m(S, B).
then(MA, MB) ->
    bind(MA, fun(_) -> MB end).

-spec return(A) -> erl_syntax_traverse_m(_S, A).
return(A) ->
    Inner = 
...
-spec '>>='(erl_syntax_traverse_m(S, A), fun((A) -> erl_syntax_traverse_m(S, B)), ?TRAVERSE_M) -> erl_syntax_traverse_m(S, B).
'>>='(MA, KMB, ?TRAVERSE_M) ->
    bind(MA, KMB).

-spec return(A, ?TRAVERSE_M) -> erl_syntax_traverse_m(_S, A).
return(A, ?TRAVERSE_M) ->
    return(A).
...
updated_node(NodeA, MB) ->
    map_m_state_ok(
      fun(#{updated := false} = MState) ->
              update_m_state(MState, #{return => NodeA, nodes => erl_syntax_endo:endo([NodeA])});
         (MState) ->
              MState
      end, MB).

-spec fail_on_error(erl_syntax_traverse_m(S, A)) -> erl_syntax_traverse_m(S, A).
fail_on_error(MA) ->
    map_m_state_ok(
      fun(#{state := State1, error := Error} = StateOk) ->
              case erl_syntax_error_state:is_empty_error(Error) of
                  true ->
                      StateOk;
                  false ->
...
-spec sequence_either([erl_syntax_traverse_m(S, A)]) -> erl_syntax_traverse_m(S, [A]).
...
sequence_either([]) ->
    erl_syntax_traverse_m:return([]).

-spec sequence_all([erl_syntax_traverse_m(S, A)]) -> erl_syntax_traverse_m(S, [A]).
sequence_all([MA|MAs]) ->
    map_m_state(
      fun(Formatter, #{state := State1, error := Error1} = MState1) ->
...
-spec fail(_E) -> erl_syntax_traverse_m(_S, _A).
fail(E) ->
    fails([E]).

-spec fail(_E, ?TRAVERSE_M) -> erl_syntax_traverse_m(_S, _A).
fail(E, ?TRAVERSE_M) ->
    fails([E]).

-spec fails([_E]) -> erl_syntax_traverse_m(_S, _A).
fails(Es) ->
    Inner =
        fun(_Formatter, State, Error0) ->
                Error1 = erl_syntax_error_state:append_errors(Es, Error0),
...
%%%===================================================================
-spec state(fun((S) -> {A, S})) -> erl_syntax_traverse_m(S, A).
state(F) ->
    Inner = 
...
-spec modify(fun((S) -> S)) -> erl_syntax_traverse_m(S, ok).
modify(F) ->
    state(fun(State) -> State1 = F(State), {ok, State1} end).

-spec get() -> erl_syntax_traverse_m(S, S).
get() ->
    state(fun(State) -> {State, State} end).

-spec put(S) -> erl_syntax_traverse_m(S, ok).
put(State) ->
...
listen(MA) ->
    map_m_state_ok(
      fun(#{return := Return, nodes := Nodes} = StateM) ->
              update_m_state(StateM, #{return => {Return, erl_syntax_endo:run(Nodes)}})
      end, MA).

listen_nodes(MA) ->
    map_m_state_ok(
      fun(#{return := Return, nodes := Nodes} = StateM) ->
              update_m_state(StateM, #{return => {Return, erl_syntax_endo:run(Nodes)}, nodes => erl_syntax_endo:empty()})
      end, MA).
...
pop_nodes(MA) ->
    map_m_state_ok(
      fun(#{nodes := Nodes} = StateM) ->
              update_m_state(StateM, #{return => erl_syntax_endo:run(Nodes), nodes => erl_syntax_endo:endo([])})
      end, MA).
...
nodes(Nodes) ->
    Inner =
        fun(_Formatter, State, Error) ->
                state_ok(#{return => ok, state => State, error => Error, nodes => erl_syntax_endo:endo(Nodes), updated => true})
        end,
    new(Inner).
%%%===================================================================
...
-spec with_error(fun((erl_syntax_error_state:erl_syntax_error_state())
                     -> erl_syntax_error_state:erl_syntax_error_state()),
                 erl_syntax_traverse_m(S, A))
                -> erl_syntax_traverse_m(S, A).
with_error(F, MA) ->
    map_m_state(
...
-spec modify_error(fun((erl_syntax_error_state:erl_syntax_error_state())
                     -> erl_syntax_error_state:erl_syntax_error_state()))
                -> erl_syntax_traverse_m(_S, _A).
modify_error(F) ->
    Inner = 
        fun(_Formatter, State, Error0) ->
                Error1 = F(Error0),
...
-spec with_formatter(fun((formatter()) -> formatter()), erl_syntax_traverse_m(S, A)) -> erl_syntax_traverse_m(S, A).
...
-spec warning(term()) -> erl_syntax_traverse_m(_S, _A).
warning(Warning) ->
    warnings([Warning]).

-spec warnings([term()]) -> erl_syntax_traverse_m(_S, _A).
warnings(Warnings) ->
    modify_error(
      fun(Error) ->
              erl_syntax_error_state:append_warnings(Warnings, Error)
      end).

-spec formatted_warnings([{line(), formatter(), term()}]) -> erl_syntax_traverse_m(_S, _A).
formatted_warnings(Warnings) ->
    modify_error(
      fun(Error) ->
              erl_syntax_error_state:append_formatted_warnings(Warnings, Error)
      end).

-spec error(term()) -> erl_syntax_traverse_m(_S, _A).
error(Error) ->
    errors([Error]).

-spec errors([term()]) -> erl_syntax_traverse_m(_S, _A).
errors(Errors) ->
    modify_error(
      fun(Error) ->
              erl_syntax_error_state:append_errors(Errors, Error)
      end).

-spec formatted_errors([{line(), formatter(), term()}]) -> erl_syntax_traverse_m(_S, _A).
formatted_errors(Errors) ->
    modify_error(
      fun(Error) ->
              erl_syntax_error_state:append_formatted_errors(Errors, Error)
      end).

-spec update_file(file:filename()) -> erl_syntax_traverse_m(_S, _A).
update_file(File) ->
    modify_error(
      fun(Error) ->
              erl_syntax_error_state:update_file(File, Error)
      end).

-spec eof() -> erl_syntax_traverse_m(_S, _A).
eof() ->
    modify_error(
      fun(Error) ->
              erl_syntax_error_state:eof(Error)
      end).

-spec update_line(line(), erl_syntax_traverse_m(S, A)) -> erl_syntax_traverse_m(S, A).
update_line(Line, MA) ->
    map_m_state(
      fun(Formatter, #{error := Error0, error := Error0} = MState) ->
              case erl_syntax_error_state:no_pending(Error0) of
                  true ->
                      MState;
                  false ->
                      Error1 = erl_syntax_error_state:update_line(Line, Formatter, Error0),
...
init_struct_value(#{} = Struct, nodes) ->
    Struct#{nodes => erl_syntax_endo:endo([])};
init_struct_value(#{} = Struct, error) ->
    Struct#{error => erl_syntax_error_state:new()};
