...
%%%-------------------------------------------------------------------
-module(erl_syntax_monad_state_t).
...
lift(MA, {?MODULE, IM}) ->
    state_t(
      fun(S) ->
              erl_syntax_monad:lift_m(fun(A) -> {A, S} end, MA, IM)
      end).
...
state(F, {?MODULE, IM}) ->
    state_t(fun (S) -> erl_syntax_monad:return(F(S), IM) end).

fail(E, ST) ->
    erl_syntax_monad:lift_fail(E, ST).

tell(Ms, ST) ->
    erl_syntax_monad:lift_tell(Ms, ST).
