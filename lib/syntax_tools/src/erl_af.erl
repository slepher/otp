...
%% <li>false = return full erl_af_return struct</li>
%% </ul></dd>
%% <dt>Attr = initial attr for traverse_attr() in traverse_fun().</dt>
...
%% </ul></dd>
%% <dt>SyntaxLib = which syntax lib is used by, default to erl_af_syntax.</dt>
...
map(F, TopAst, Opts) ->
    F1 = fun(Node, _State, Attr) ->
                 ...
                 erl_af_walk_return:new(Concereter, Return)
         end,
...
reduce(F, Acc0, TopAst, Opts) ->
    NF = fun(Node, State, Attr) ->
                 ...
                 erl_af_walk_return:new(fun() -> #{state => Return} end, Return)
         end,
...
mapfold(F, Init, TopAst, Opts) ->
    ...
    NF = erl_af_traverse_m:transform_mapfold_f(F),
    TopAstM = map_m(NF, TopAst, Opts1),
    Return = erl_af_traverse_m:run(TopAstM, Formatter, Init),
...
transform_return(Return, Opts) ->
    Return1 =
        case maps:find(transform, Opts) of
            {ok, Transform} ->
                erl_af_monad:lift_m(Transform, Return);
            error ->
                Return
        end,
    case maps:get(parse_transform, Opts, false) of
        true ->
            erl_af_return:to_compiler(Return1);
        false ->
            case maps:get(simplify_return, Opts, true) of
                true ->
                    Return2 = erl_af_return:simplify(Return1),
                    Return2;
                false ->
                    Return1
            end
...
-spec map_m(traverse_fun(), Node, traverse_opts()) -> erl_af_return:struct(Node).
...
map_forms(Fun, Forms, Opts) ->
    ...
    map_forms(Fun, Forms, Functions, erl_af_traverse_m:return(grforms_new()), Opts).
...
map_forms(Fun, [Form|Tails], Functions0, GRFormsM, Opts) ->
    erl_af_traverse_m:bind(
      ...
      fun(GRForms) ->
              erl_af_traverse_m:bind(
                erl_af_traverse_m:listen_updated(erl_af_traverse_m:catched_nodes(apply_fun(Fun, Form, Opts))),
                fun({_, false}) ->
                        map_forms(Fun, Tails, Functions0, grforms_appendm(Form, GRFormsM), Opts);
                   ({NewForm, true}) ->
                        NewForms = to_list(NewForm),
                        ...
                        map_forms(Fun, Tails2, Functions1, erl_af_traverse_m:return(GRForms1), Opts)
                end)
      end);
map_forms(_Fun, [], _Acc, FormsM, _Opts) ->
    erl_af_monad:lift_m(fun grforms_to_forms/1, FormsM).
    ...
    TraverseM =
        case apply_f(Fun, Form) of
            ...
            #{?STRUCT_KEY := ?RETURN_FAIL} = Return ->
                erl_af_traverse_m:erl_af_traverse_m(Return);
            Return ->
                erl_af_traverse_m:erl_af_traverse_m(erl_af_walk_return:new(Return))
        end,
    erl_af_traverse_m:update_line(SyntaxLib:get_pos(Form), TraverseM);
...
grforms_appendm(Form, GRFormsM) ->
    erl_af_monad:lift_m(fun(GRForms) -> grforms_append(Form, GRForms) end, GRFormsM).
...
up_f_by_traverse(F, #{traverse := pre}) ->
    fun(Node, #{step := pre  } = Attr) -> F(Node, Attr);
       ...
       (Node, #{step := post }) -> erl_af_traverse_m:return(Node)
    end;
up_f_by_traverse(F, #{traverse := post}) ->
    fun(Node, #{step := pre  }) -> erl_af_traverse_m:return(Node);
       (Node, #{step := leaf } = Attr) -> F(Node, Attr);
...
map_m_1(F, NodeA, #{children := true} = Opts) ->
    ...
    Subtrees = SyntaxLib:subtrees(NodeA, Attr),
    Sequence = maps:get(sequence_children, Opts, fun erl_af_traverse_m:deep_sequence_nodes/1),
    map_m_children(F, NodeA, Subtrees, Attr, Sequence, SyntaxLib);
map_m_1(F, NodeA, #{} = Opts) ->
    SyntaxLib = syntax_lib(Opts),
    Attr = maps:get(attr, Opts, #{}),
    map_m_tree_node(F, NodeA, Attr, SyntaxLib).

map_m_tree_node(F, NodeA, Attr, SyntaxLib) ->
    %% it's not a good way to mix attr with opts.
    NodeType = SyntaxLib:node_type(NodeA, Attr),
    PreType =
        case SyntaxLib:subtrees(NodeA, Attr) of
            [] ->
                leaf;
            _Subtrees ->
                pre
    ...
    bind_continue(
      ...
      fun(NodeB, Sequence) ->
              case SyntaxLib:tps(NodeB, Attr) of
                  {_Type, _Pos, []} ->
                      erl_af_traverse_m:return(NodeB);
                  {Parent, _Pos, Subtrees} ->
                      erl_af_traverse_m:bind(
                        map_m_children(F, NodeB, Subtrees, Attr#{parent => Parent}, Sequence, SyntaxLib),
                        fun(NodeC) ->
                                erl_af_traverse_m:set_continue(
                                  apply_f(F, NodeC, Attr#{step => post, node => NodeType}, SyntaxLib), false)
                        end)
              end
      end).

apply_f(F, NodeA, Attr, SyntaxLib) ->
    Line = SyntaxLib:get_pos(NodeA),
    erl_af_traverse_m:lift_m(
      fun({_NodeB, false}) ->
              NodeA;
         ({NodeB, true}) ->
              NodeB
      end, erl_af_traverse_m:listen_updated(erl_af_traverse_m:update_line(Line, F(NodeA, Attr)))).

bind_continue(NodeAM, AMB) ->
    erl_af_traverse_m:bind(
      erl_af_traverse_m:listen_continue(NodeAM),
      fun({NodeA, true}) ->
              erl_af_traverse_m:return(NodeA);
         ({NodeAs, Continue}) when is_list(NodeAs) ->
              Sequence = update_sequence_children(Continue),
              erl_af_traverse_m:sequence_nodes(lists:map(fun(Node) -> AMB(Node, Sequence) end, NodeAs));
         ({NodeA, Continue}) ->
              Sequence = update_sequence_children(Continue),
              AMB(NodeA, Sequence)
      end).

update_sequence_children(false) ->
    fun erl_af_traverse_m:deep_sequence_nodes/1;
update_sequence_children(Sequence) ->
    Sequence.

map_m_children(_F, Node, [], _Attr,  _Sequence, _SyntaxLib) ->
    erl_af_traverse_m:return(Node);

map_m_children(F, Node, Subtrees, Attr, Sequence, SyntaxLib) ->
    SyntaxType = SyntaxLib:type(Node),
    SubtreeMs = m_subtrees(F, Subtrees, Attr#{parent => SyntaxType}, SyntaxLib),
    SubtreesM = Sequence(SubtreeMs),
    erl_af_traverse_m:bind(
      erl_af_traverse_m:listen_updated(SubtreesM),
      fun({_Subtrees1, false}) ->
              erl_af_traverse_m:return(Node);
         ({Subtrees1, true}) ->
              Node1 = SyntaxLib:update_subtrees(Node, Subtrees1),
              erl_af_traverse_m:return(Node1)
      end).
...
m_subtrees(_F, {skip, Subtrees}, _Attr, _SyntaxLib) when is_list(Subtrees) ->
    lists:map(
      fun(Subtree) ->
              erl_af_traverse_m:return(Subtree)
      end, Subtrees);
m_subtrees(_F, {skip, Subtree}, _Attr, _SyntaxLib) ->
    erl_af_traverse_m:return(Subtree);
m_subtrees(F, {transformer, Subtree, Transformer}, Attr, SyntaxLib) ->
    erl_af_traverse_m:bind(
      erl_af_traverse_m:sequence_nodes(m_subtrees(F, Subtree, Attr, SyntaxLib)),
      fun(Subtree1) ->
              Transformed = Transformer(Subtree1),
              erl_af_traverse_m:return(Transformed)
      end);
m_subtrees(F, {up_attr, UpAttr, Subtree}, Attr, SyntaxLib) when is_map(Attr) ->
    Attr1 = maps:merge(Attr, UpAttr),
    m_subtrees(F, Subtree, Attr1, SyntaxLib);
m_subtrees(F, {up_node, Node, Subtree}, Attr, SyntaxLib) when is_atom(Node) ->
    Attr1 = Attr#{node => Node},
    m_subtrees(F, Subtree, Attr1, SyntaxLib);
m_subtrees(F, Subtrees, Attr, SyntaxLib) when is_list(Subtrees) ->
    lists:map(
      fun(Subtree) ->
              m_subtrees(F, Subtree, Attr, SyntaxLib)
      end, Subtrees);
m_subtrees(F, Subtree, Attr, SyntaxLib) ->
    map_m_tree_node(F, Subtree, Attr, SyntaxLib).
