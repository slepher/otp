...
%%%-------------------------------------------------------------------
-module(erl_syntax_error_state).

-include("erl_syntax_struct_name.hrl").

-export_type([erl_syntax_error/0]).
-export_type([compiler_error/0]).

-opaque erl_syntax_error() :: #{'__struct__' => ?MODULE,
                               file => file(),
                               errors => erl_syntax_endo:endo(error()),
                               warnings => erl_syntax_endo:endo(error()),
                               formatted_errors => erl_syntax_endo:endo(formatted_error()),
                               formatted_warnings => erl_syntax_endo:endo(formatted_error()),
                               file_errors => #{file() => erl_syntax_endo:endo(formatted_error())},
                               file_warnings => #{file() => erl_syntax_endo:endo(formatted_error())}
                              }.
...
new(File) ->
    #{?STRUCT_KEY => ?MODULE, file => File,
      errors => erl_syntax_endo:empty(), warnings => erl_syntax_endo:empty(),
      formatted_errors => erl_syntax_endo:empty(), formatted_warnings => erl_syntax_endo:empty(),
...
update_line(Line, Formatter, #{?STRUCT_KEY := ?MODULE,
    ...
    FormattedErrors2 = erl_syntax_endo:append(FormattedErrors0, FormattedErrors1),
    FormattedWarnings2 = erl_syntax_endo:append(FormattedWarnings0, FormattedWarnings1),
...
is_empty(#{?STRUCT_KEY := ?ERROR_STATE,
  ...
  when (map_size(FErrors) == 0) and (map_size(FWarnings) == 0) ->
    erl_syntax_endo:is_empty(Errors) and erl_syntax_endo:is_empty(Warnings)
        and erl_syntax_endo:is_empty(FormattedErrors) and erl_syntax_endo:is_empty(FormattedWarnings);
...
is_empty_error(#{errors := Errors,
  ...
  when (map_size(FErrors) == 0) ->
    erl_syntax_endo:is_empty(Errors) and erl_syntax_endo:is_empty(FormattedErrors);
...
realize_errors(FileErrors) ->
    lists:map(fun({File, Errors}) -> {File, erl_syntax_endo:run(Errors)} end, maps:to_list(FileErrors)).

run_endo(#{?STRUCT_KEY := ?ERROR_STATE,
           errors := Errors, warnings := Warnings,
           formatted_errors := FormattedErrors, formatted_warnings := FormattedWarnings,
           file_errors := FileErrors, file_warnings := FileWarnings} = State) ->
    State#{errors => erl_syntax_endo:run(Errors), warnings => erl_syntax_endo:run(Warnings),
           formatted_errors => erl_syntax_endo:run(FormattedErrors), formatted_warnings => erl_syntax_endo:run(FormattedWarnings),
...
errors(#{errors := Errors}) ->
    erl_syntax_endo:run(Errors).

warnings(#{warnings := Warnings}) ->
    erl_syntax_endo:run(Warnings).
...
update_file(File0, File1, #{?STRUCT_KEY := ?ERROR_STATE,
    ...
    case File0 == File1 of
        ...
        false ->
            ...
            State#{formatted_errors => erl_syntax_endo:empty(),
                   formatted_warnings => erl_syntax_endo:empty(),
...
add_file_errors(File, Errors1, ErrorsWithFile) ->
    case erl_syntax_endo:is_empty(Errors1) of
        true ->
            ErrorsWithFile;
        false ->
...
append(Errors0, Errors1) ->
    erl_syntax_endo:append(Errors0, Errors1).

format_errors(Line, Formatter, Errors) ->
    lists:map(fun(Error) -> {Line, Formatter, Error} end, erl_syntax_endo:run(Errors)).
