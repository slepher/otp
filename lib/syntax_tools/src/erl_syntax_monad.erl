...
foldl_m(_F, Acc, [], Monad) ->
    erl_syntax_monad:return(Acc, Monad).
...
lift_get({_, IM} = WT) ->
    lift(erl_syntax_monad:get(IM), WT).

lift_put(S, {_, IM} = WT) ->
    lift(erl_syntax_monad:put(S, IM), WT).

lift_state(F, {_, IM} = WT) ->
    lift(erl_syntax_monad:state(F, IM), WT).
