...
%%% @doc
%%% struct of error created by {@link erl_abs:mapfold/4}.
%%% @end
%%% Created :  9 Jul 2020 by Chen Slepher <slepheric@gmail.com>
%%%-------------------------------------------------------------------
-module(erl_abs_error).
...
-type struct() :: #{'__struct__' => ?MODULE,
                    file => compile_file(),
                    errors => erl_abs_endo:endo(term()),
                    warnings => erl_abs_endo:endo(term()),
                    formatted_errors => erl_abs_endo:endo(erl_parse:error_info()),
                    formatted_warnings => erl_abs_endo:endo(erl_parse:error_info()),
                    file_errors => #{file:filename() => erl_abs_endo:endo(erl_parse:error_info())},
                    file_warnings => #{file:filename() => erl_abs_endo:endo(erl_parse:error_info())}
                   }.
...
%% get memebers from erl_abs_error:struct().
...
%% append errors to erl_abs_error:struct().
...
new(File) ->
    #{?STRUCT_KEY => ?MODULE, file => File,
      errors => erl_abs_endo:empty(), warnings => erl_abs_endo:empty(),
      formatted_errors => erl_abs_endo:empty(), formatted_warnings => erl_abs_endo:empty(),
...
update_line(Line, Formatter, #{?STRUCT_KEY := ?MODULE,
    ...
    FormattedErrors2 = erl_abs_endo:append(FormattedErrors0, FormattedErrors1),
    FormattedWarnings2 = erl_abs_endo:append(FormattedWarnings0, FormattedWarnings1),
...
no_pending(#{?STRUCT_KEY := ?ERROR_STATE, errors := Errors, warnings := Warnings}) ->
    erl_abs_endo:is_empty(Errors) and erl_abs_endo:is_empty(Warnings).
...
is_empty(#{?STRUCT_KEY := ?ERROR_STATE,
  ...
  when (map_size(FErrors) == 0) and (map_size(FWarnings) == 0) ->
    erl_abs_endo:is_empty(Errors) and erl_abs_endo:is_empty(Warnings)
    and erl_abs_endo:is_empty(FormattedErrors) and erl_abs_endo:is_empty(FormattedWarnings);
...
is_empty_error(#{errors := Errors,
  ...
  when (map_size(FErrors) == 0) ->
    erl_abs_endo:is_empty(Errors) and erl_abs_endo:is_empty(FormattedErrors);
        ...
        fun(Key, FileErrors, Acc) when (Key == file_errors) or (Key == file_warnings) ->
            case maps:size(FileErrors) of
                0 ->
                    Acc;
                _ ->
                    FileErrors1 = maps:map(fun(_Key, Value) -> erl_abs_endo:run(Value) end, FileErrors),
           ...
           (Key, Errors, Acc) when (Key == formatted_errors) or (Key == formatted_warnings)
               ...
               case erl_abs_endo:is_empty(Errors) of
                    true ->
                       Acc;
                    false ->
                        maps:put(Key, erl_abs_endo:run(Errors), Acc)
                end;
           (_Key, _Value, Acc) ->
               Acc
        end, #{}, Struct).
...
%% @doc merge two erl_abs_error struct into one, first update file of Struct1 to file of Struct2, then merge all errors.
-spec merge(struct(), struct()) -> struct().
...
with_error(Fun, Struct) ->
   with_key(Fun, errors, fun erl_abs_endo:map/2, Struct).

with_warning(Fun, Struct) ->
   with_key(Fun, warnings, fun erl_abs_endo:map/2, Struct).

with_failure(Fun, Struct) ->
   erl_abs_monad:sequence_withs(Fun, [fun with_error/2, fun with_warning/2], Struct).

with_formatted_error(Fun, Struct) ->
   with_key(Fun, formatted_errors, fun erl_abs_endo:map/2, Struct).

with_formatted_warning(Fun, Struct) ->
   with_key(Fun, formatted_warnings, fun erl_abs_endo:map/2, Struct).

with_formatted_failure(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_formatted_error/2, fun with_formatted_warning/2], Struct).
...
with_formatted_base_failure(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_formatted_base_error/2, fun with_formatted_base_warning/2], Struct).

with_formatted(Fun, {Line, Formatter, Error}) ->
    {Line, Formatter, Fun(Error)}.

with_formatted_base(Fun, FormattedErrors) ->
    erl_abs_monad:nested_withs(Fun, [fun with_formatted/2, fun erl_abs_endo:map/2], FormattedErrors).
...
with_file_failures(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_file_errors/2, fun with_file_warnings/2], Struct).
...
with_file_formatted_failure(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_file_formatted_error/2, fun with_file_formatted_warning/2], Struct).
...
with_file_base_failure(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_file_base_error/2, fun with_file_base_warning/2], Struct).

with_file(Fun, FileErrors) ->
    maps:from_list(lists:map(Fun, maps:to_list(FileErrors))).

with_file_formatted(Fun, FileErrors) ->
    erl_abs_monad:nested_withs(Fun, [fun erl_abs_endo:map/2, fun with_map_value/2], FileErrors).

with_file_base(Fun, FileErrors) ->
    erl_abs_monad:nested_withs(Fun, [fun with_formatted/2, fun with_file_formatted/2], FileErrors).

with_all_error(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_error/2, fun with_formatted_base_error/2, fun with_file_base_error/2], Struct).

with_all_warning(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_warning/2, fun with_formatted_base_warning/2, fun with_file_base_warning/2], Struct).

with_all_failure(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_all_error/2, fun with_all_warning/2], Struct).

with_all_formatted_error(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_formatted_error/2, fun with_file_formatted_error/2], Struct).

with_all_formatted_warning(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_formatted_warning/2, fun with_file_formatted_warning/2], Struct).

with_all_formatted_failure(Fun, Struct) ->
    erl_abs_monad:sequence_withs(Fun, [fun with_all_formatted_error/2, fun with_all_formatted_warning/2], Struct).
...
%% =====================================================================
%% @spec append_ews(erl_abs_endo:list_or_endo(term()), erl_abs_endo:list_or_endo(term()), struct()) -> struct()
...
%% =====================================================================
%% @spec append_errors(erl_abs_endo:list_or_endo(term()), struct()) -> struct()
...
%% =====================================================================
%% @spec append_warnings(erl_abs_endo:list_or_endo(term()), struct()) -> struct()
...
%% =====================================================================
%% @spec append_formatted_errors(erl_abs_endo:list_or_endo(erl_parse:parse_error()), struct()) -> struct()
...
%% =====================================================================
%% @spec append_formatted_warnings(erl_abs_endo:list_or_endo(erl_parse:parse_error()), struct()) -> struct()
...
realize_errors(FileErrors) ->
    lists:map(fun({File, Errors}) -> {File, erl_abs_endo:run(Errors)} end, maps:to_list(FileErrors)).
...
update_file(File0, File1, #{?STRUCT_KEY := ?ERROR_STATE,
    ...
    case File0 == File1 of
        ...
        false ->
            ...
            Struct#{formatted_errors => erl_abs_endo:empty(),
                    formatted_warnings => erl_abs_endo:empty(),
...
merge_file_errors(ErrorsWithFile1, ErrorsWithFile2) ->
 ...
 add_file_errors(File, Errors1, ErrorsWithFile) ->
    case erl_abs_endo:is_empty(Errors1) of
        true ->
            ErrorsWithFile;
        false ->
...
append(Errors0, Errors1) ->
    erl_abs_endo:append(Errors0, Errors1).

format_errors(Line, Formatter, Errors) ->
    lists:map(fun(Error) -> {Line, Formatter, Error} end, erl_abs_endo:run(Errors)).

from_compiler_errors(CompilerFileErrors) ->
    maps:from_list(
      lists:map(
        fun({File, Errors}) ->
                {File, erl_abs_endo:endo(Errors)}
