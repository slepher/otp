...
%%% @doc
%%% return type of traverse fun in {@link erl_af:map/3}, {@link erl_af:reduce/4}, {@link erl_af:map_with_state/4}, {@link erl_af:mapfold/4}.
%%% @end
%%% Created : 10 Jul 2020 by Chen Slepher <slepheric@gmail.com>
%%%-------------------------------------------------------------------
-module(erl_af_walk_return).
...
%% @doc generate erl_af_walk_return:struct(S, A) from walk function return in erl_af:map/3 erl_af:reduce/4, erl_af:map_with_state/4, erl_af:mapfold/4
%% if return type is not recognized, use DefaultConcereter to generate a map which could converted to erl_af_walk_return:struct(S, A).
%% @end
...
new(Concereter, Return) ->
    case erl_af_lib:try_concerete(Return, [fun erl_af_lib:base_concereter/1, fun walk_return_concereter/1]) of
        {ok, Map} ->
            new(Map);
        error ->
            case Concereter() of
...
up_map(#{nodes := Nodes} = Map) ->
    case erl_af_lib:validate_node(Nodes) of
        ok ->
            Map;
        {error, Reason} ->
...
up_map(#{node := Node} = Map) ->
    up_map(maps:remove(node, Map#{nodes => Node}));
up_map(#{} = Map) ->
    Map.
