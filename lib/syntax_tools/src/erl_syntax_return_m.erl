...
%%%-------------------------------------------------------------------
-module(erl_syntax_return_m).

-include("erl_syntax_struct_name.hrl").

-export_type([erl_syntax_return_m/1]).

-opaque erl_syntax_return_m(A) :: 
          #{?STRUCT_KEY => ?RETURN_OK,
            return => A,
            error => erl_syntax_error_state:new()} |
...
%%%===================================================================
return_ok(Return) ->
    return_ok(Return, erl_syntax_error_state:new()).
...
from_compiler_errors(CompilerFileErrors) ->
    maps:from_list(
      lists:map(
        fun({File, Errors}) ->
                {File, erl_syntax_endo:endo(Errors)}
...
-spec to_compiler(erl_syntax_return_m([erl_syntax:node()])) ->
                         [erl_syntax:node()] | {warning, [erl_syntax:node()], erl_syntax_error_state:compiler_error()} |
                         {error, erl_syntax_error_state:compiler_error(), erl_syntax_error_state:compiler_error()}.
to_compiler(#{?STRUCT_KEY := ?RETURN_OK, return := Forms, error := Error}) ->
    case erl_syntax_error_state:realize(Error) of
        {[], []} ->
            Forms;
        {[], Warnings} ->
...
to_compiler(#{?STRUCT_KEY := ?RETURN_FAIL, error := Error}) ->
    {Errors, Warnings} = erl_syntax_error_state:realize(Error),
    {error, Errors, Warnings}.

simplify(#{?STRUCT_KEY := ?RETURN_OK, return := Return, error := Error} = MA) ->
    case erl_syntax_error_state:is_empty(Error) of
        true ->
            Return;
        false ->
            MA
