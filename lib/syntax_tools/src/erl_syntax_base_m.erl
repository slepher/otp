...
%%%-------------------------------------------------------------------
-module(erl_syntax_base_m).

-include("erl_syntax_struct_name.hrl").

-erlando_type(?MODULE).
-erl_syntax_future_behaviour(monad).

%% Types
-export_type([erl_syntax_base_m/1]).
-opaque erl_syntax_base_m(A) :: #{'__struct__' => ?MODULE, return => A, errors => [any()], warnings => [any()]}.

%% API
-export([erl_syntax_base_m/1, to_monad/1]).
-export([new/0, run/1]).
...
%%%===================================================================
%%% Construct erl_syntax_base_m by map
%%%===================================================================
erl_syntax_base_m(#{?STRUCT_KEY := ?BASE_M} = BaseM) ->
    BaseM;
erl_syntax_base_m(#{?STRUCT_KEY := ?WALK_RETURN} = Walk) ->
    default(maps:remove(?STRUCT_KEY, Walk)).
%%%===================================================================
%%% convert traverse return value to erl_syntax_base_m
%%%===================================================================
to_monad(A) ->
    to_monad(ok, A).

-spec to_monad(A, term()) -> erl_syntax_base_m(A).
to_monad(A, Return) ->
    Return1 = 
