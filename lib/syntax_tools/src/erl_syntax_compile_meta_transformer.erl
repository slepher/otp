...
%%%-------------------------------------------------------------------
-module(erl_syntax_compile_meta_transformer).
...
fold_transformers(File, Transformers, Forms, Opts) ->
    to_compiler(
      erl_syntax_return_m:to_compiler(
        erl_syntax_monad:foldl_m(
          fun(Transformer, FormsAcc) ->
                  erl_syntax_return_m:to_monad(
                    apply_transformer(File, Transformer, FormsAcc, Opts))
          end, Forms, Transformers, erl_syntax_return_m))).
...
append_forms(Forms, OrigForms, Errors, Warnings, Opts) ->
    ...
    erl_syntax_traverse_lib:reorder_attributes(
...
base_forms([], OrigForms) ->
    ...
    [FileAttr|_T0] = erl_syntax_traverse_lib:attribute_nodes(file, OrigForms),
    [ModuleAttr|_T1] = erl_syntax_traverse_lib:attribute_nodes(module, OrigForms),
...
meta_fun_forms(MetaFun, Forms) ->
    try erl_syntax_traverse_lib:abstract(Forms) of
        AbsForms ->
            erl_syntax_traverse_lib:exported_function(
              MetaFun, {'fun', 0, {clauses, [{clause, 0, [], [], [AbsForms]}]}})
    catch
        _:_Exception ->
            exit(Forms)
    end.
